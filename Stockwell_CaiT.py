...
x_train_transformed = np.concatenate((np.apply_along_axis(lambda arr: st.st(arr, 8, 30, 0.95).real, 2, x_train),
                                      np.apply_along_axis(lambda arr: st.st(arr, 8, 30, 0.95).imag, 2, x_train)), axis=2)

x_test_transformed = np.concatenate((np.apply_along_axis(lambda arr: st.st(arr, 8, 30, 0.95).real, 2, x_test),
                                     np.apply_along_axis(lambda arr: st.st(arr, 8, 30, 0.95).imag, 2, x_test)), axis=2)
...
for density in [10, 20]:
    for segment_size in [2, 5]:
        ...
        required_padding = (segment_size - (densified_train.shape[3] % segment_size)) % segment_size
        x_train_transformed_dense = np.pad(densified_train, ((0, 0), (0, 0), (0, 0), (0, required_padding)), mode='constant', constant_values=0)
        x_test_transformed_dense = np.pad(densified_test, ((0, 0), (0, 0), (0, 0), (0, required_padding)), mode='constant', constant_values=0)
            ...
            for dropout in [0.1, 0.2]:
                for emb_dropout in [0.1, 0.2]:
                    for layer_dropout in [0.1, 0.2]:
                        ...
                        total_acc, individual_accs, total_var, individual_vars = fit_predict(v, x_train_transformed_dense, y_train, x_test_transformed_dense, y_test, IDs, epochs=10, crossval=10)
